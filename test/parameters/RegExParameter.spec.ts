...
describe("RegExParameters.spec.ts", () => {
  describe("RegexParameter+RestParameter", () => {
    it("Capture name and description", () => {
      var p = [
        new RegExParameter("name", "[^ ]+"),
        new RestParameter("description", ""),
      ]

      var r = createRegex(p)

      expect(
        test(r, "hubot test cmd name-of-incident description of the incident")
      ).to.eq(true)

      let result = exec(
        r,
        "hubot test cmd name-of-incident description of the incident"
      )
